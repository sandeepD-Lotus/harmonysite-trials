...
const Navigation = () => {
  ...
  return <nav className="fixed top-0 left-0 right-0 z-50 bg-harmony-navy/95 backdrop-blur-sm border-b border-harmony-cyan/20 animate-fade-in">
      <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8">
        ...
        <div className="flex items-center justify-between h-16 md:h-20 md:gap-8">
          {/* Logo block */}
          <div className="flex items-center space-x-2 group md:space-x-3 z-[55]">
            <Link to="/" className="flex items-center space-x-2 group md:space-x-3">
              <div className="relative overflow-hidden rounded-lg h-10 w-10 md:w-16 md:h-16 flex-shrink-0">
                <img alt="Harmony Consulting Solutions Logo" className="w-full h-full transition-transform duration-300 group-hover:scale-110 object-cover" src="/lovable-uploads/1542e19e-178f-429f-a50e-1a45224839b5.jpg" />
              ...
              <div className="flex flex-col leading-none">
                <span className="text-white font-bold text-base md:text-xl whitespace-nowrap leading-tight">HARMONY</span>
                <span className="text-harmony-cyan text-xs md:text-sm font-medium leading-tight whitespace-nowrap">CONSULTING SOLUTIONS</span>
              </div>
            </Link>
          </div>
          {/* Desktop Navigation Links */}
          <div className="hidden md:flex flex-1 items-center justify-center space-x-8 min-w-0 overflow-x-auto">
            {navItems.map((item, index) => <div key={item.name} className="relative" style={{
            animationDelay: `${index * 0.1}s`
          }}>
                {item.hasDropdown ? <div className="relative animate-fade-in" onMouseEnter={() => setShowSolutionsDropdown(true)} onMouseLeave={() => setShowSolutionsDropdown(false)}>
                    <Link to={item.path} className={`text-sm font-medium transition-all duration-300 hover:text-harmony-cyan flex items-center relative after:content-[''] after:absolute after:w-full after:scale-x-0 after:h-0.5 after:bottom-0 after:left-0 after:bg-harmony-cyan after:origin-bottom-right after:transition-transform after:duration-300 hover:after:scale-x-100 hover:after:origin-bottom-left ${location.pathname === item.path ? 'text-harmony-cyan after:scale-x-100' : 'text-white'}`}>
                      {item.name}
                      <ChevronDown className="w-4 h-4 ml-1 transition-transform duration-300 group-hover:rotate-180" />
                    </Link>
                    {showSolutionsDropdown && <div className="absolute top-full left-0 mt-2 w-64 bg-harmony-navy border border-harmony-cyan/30 rounded-lg shadow-xl z-10 animate-scale-in">
                        <div className="py-2">
                          {solutionItems.map((solutionItem, idx) => <Link key={idx} to={solutionItem.path} className="block px-4 py-2 text-sm text-gray-300 hover:text-harmony-cyan hover:bg-harmony-navy/50 transition-all duration-200 hover:translate-x-2">
                              {solutionItem.name}
                            </Link>)}
                        </div>
                      </div>}
                  </div> : <Link to={item.path} className={`text-sm font-medium transition-all duration-300 hover:text-harmony-cyan relative after:content-[''] after:absolute after:w-full after:scale-x-0 after:h-0.5 after:bottom-0 after:left-0 after:bg-harmony-cyan after:origin-bottom-right after:transition-transform after:duration-300 hover:after:scale-x-100 hover:after:origin-bottom-left animate-fade-in ${location.pathname === item.path ? 'text-harmony-cyan after:scale-x-100' : 'text-white'}`}>
                    {item.name}
                  </Link>}
              </div>)}
          </div>
          {/* Desktop CTA Button (right aligned, never wraps) */}
        ...
        {showMobileMenu && <>
          ...
          {/* Mobile Menu */}
          <div className="md:hidden fixed top-0 left-0 right-0 z-[60] bg-harmony-navy w-full pt-4 min-h-screen flex flex-col animate-scale-in shadow-xl border-r border-harmony-cyan/40" style={{
          maxWidth: '100%',
          ...
          borderTopRightRadius: 0
            ...
            <div className="flex justify-between items-start px-4 pb-1 w-full bg-harmony-navy z-10">
              ...
              <Link to="/" className="flex items-center space-x-2 group">
                <div className="relative overflow-hidden rounded-lg h-8 w-8 bg-white">
                  <img src="/lovable-uploads/5293c8eb-5f72-4bb9-b36a-732d2afdec1d.png" alt="Harmony Consulting Solutions Logo" className="w-full h-full transition-transform duration-300 group-hover:scale-110 object-cover" />
                  <div className="absolute inset-0 bg-harmony-cyan/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                </div>
                <div className="flex-col ml-2">
                  <span className="text-white font-bold text-sm leading-tight" style={{
                  whiteSpace: 'nowrap'
                }}>
                    HARMONY
                  </span>
                  <div className="text-harmony-cyan text-[0.6rem] font-medium leading-tight" style={{
                  whiteSpace: 'nowrap'
                }}>
            ...
            <nav className="flex flex-col gap-0 pt-2 pb-6" style={{}}>
              {navItems.map((item, idx) => item.hasDropdown ? <div key={item.name} className="w-full">
                    <button onClick={() => setMobileSolutionsOpen(open => !open)} className={`
                        ...
                        `} aria-expanded={mobileSolutionsOpen} style={{
                minHeight: '44px',
                lineHeight: '1.3',
                letterSpacing: '-0.01em'
                    ...
                    {mobileSolutionsOpen && <div className="bg-harmony-navy/95">
                        {solutionItems.map(solutionItem => <Link key={solutionItem.name} to={solutionItem.path} onClick={closeMobileMenu} className="block pl-10 pr-5 py-2 text-sm border-b border-harmony-cyan/10 text-gray-300 hover:text-harmony-cyan hover:bg-harmony-navy/70" style={{
                  lineHeight: '1.2',
                  letterSpacing: '-0.01em'
                }}>
                            {solutionItem.name}
                  ...
                  </div> : <Link key={item.name} to={item.path} onClick={closeMobileMenu} className={`
